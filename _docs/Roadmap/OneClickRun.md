...
A one click run pre-configured CodeGlass to profile a specific application with a set of [presets](ConfigPresets.md).

## Main Reason of Feature
Eliminate non reproducible issues

## Use Case Example
The customer calls that the app crashes when she tries to log in.
You try it on your machine and it does not happen.

You create a one click run with the login [preset](ConfigPresets.md) configuration in CodeGlass.
